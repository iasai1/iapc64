...
# Compiled at: 2023-06-30 15:18:52
...
class TouchStripElement(EncoderElement):
    ...
    def _update_parameter_listeners(self):
        ...
        self._TouchStripElement__on_selected_track_changed.subject = song().view
    ...
    def __on_track_color_index_changed(self):
        self.send_midi((
         CC_STATUS,
         self._led_color_cc,
         make_color_for_liveobj(self._track,
             track_index(self._track) == track_index(None)
             ).midi_value))
        
    @listens('selected_track')
    def __on_selected_track_changed(self):
        eq = track_index(self._track) == track_index(None)
        index = track_index(self._track)

        sys.stderr.write(f"strip{index}\tselected{eq}\t{str(self._track)}\n")
        self.send_midi((
         CC_STATUS,
         self._led_color_cc,
         make_color_for_liveobj(self._track,
